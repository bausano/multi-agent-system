...
class PredatorPreyPursuitAlpha(gym.Env):
    ...
    # @param observable_data                Contains the latest message from the
    #                                       MC adapter about state and reward
    # @param actor_direction                Actor's actions should be stored in
    #                                       this tuple as `(x, z)` where both
    #                                       x, z belong to [-1; 1].
    #                                       We don't use the height (y coord).
    def __init__(
        self,
        ...
        self.actor_direction = actor_direction
    ...
    def step(self, action):
        ...
        observation = self.observable_data.copy()
        # invalidate the data for the next loop iteration, wait until WS thread
        # again receives new data
        self.observable_data_ready_event.clear()

        reward = observation["reward"]
        # TODO: transform into one long vector of numbers?
        state = {
